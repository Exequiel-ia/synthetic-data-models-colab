...
from .tabula_utils import _convert_tokens_to_dataframe, _array_to_dataframe, _get_column_distribution, _convert_tokens_to_text, \
...
class Tabula:
    ...
    def __init__(self, llm: str, experiment_dir: str = "trainer_tabula", epochs: int = 100,
                 batch_size: int = 8, columns: dict = None, num_cols: dict = None, conditional_col: dict = None, **train_kwargs):
        """ Initializes Tabula.
    ...
    def fit(self, data: tp.Union[pd.DataFrame, np.ndarray], column_names: tp.Optional[tp.List[str]] = None,
        ...
        tabula_trainer = TabulaTrainer(self.model, training_args, train_dataset=tabula_ds, processing_class=self.tokenizer,
    ...
    def save(self, path: str):
        ...
        with fs.open(path + "/config.json", "w") as f:
            ...
            # Hugging Face configs -> dict (JSON-safe)
            if isinstance(attributes.get("config"), PretrainedConfig):
                attributes["config"] = attributes["config"].to_dict()

            # NDArray -> list (JSON-safe)
            if isinstance(attributes.get("conditional_col_dist"), np.ndarray):
                attributes["conditional_col_dist"] = attributes["conditional_col_dist"].tolist()

            # numpy escalares en dicts (p. ej., distribución categórica)
            if isinstance(attributes.get("conditional_col_dist"), dict):
                def _py(v):
                    return v.item() if isinstance(v, (np.integer, np.floating)) else v
                attributes["conditional_col_dist"] = {str(k): _py(v) for k, v in attributes["conditional_col_dist"].items()}

            # LabelEncoders -> clases serializables
            if attributes.get("label_encoder_list"):
                serial_le = []
                for item in attributes["label_encoder_list"]:
                    le = item["label_encoder"]
                    serial_le.append({
                        "column": item["column"],
                        "classes": le.classes_.tolist()
                    })
                attributes["label_encoder_list"] = serial_le
    ...
    def load_from_dir(cls, path: str):
        ...
        # Rebuild LabelEncoders from serialized classes if present
        if isinstance(getattr(tabula, "label_encoder_list", None), list) and tabula.label_encoder_list:
            rebuilt = []
            for item in tabula.label_encoder_list:
                le = preprocessing.LabelEncoder()
                le.fit(np.array(item["classes"], dtype=object))
                rebuilt.append({"column": item["column"], "label_encoder": le})
            tabula.label_encoder_list = rebuilt

        # Load model weights
        tabula.model.load_state_dict(torch.load(fs.open(path + "/model.pt", "rb"), map_location="cpu"))
